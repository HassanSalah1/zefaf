...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card-box">
                <a type="button" href="{{url('/event/add')}}"
                        class="btn btn-info btn-rounded w-md waves-effect waves-light m-b-5">
                        <i class="fa fa-plus-square"></i> {{trans('admin.add_event')}}
                </a>
            </div>
        </div>
    </div>
...
@section('script')
    ...
    <script>
        ...
        $(function () {
            ...
            loadDataTables('{{ url( (($locale === 'ar') ? $locale : '') ."/events/data") }}',
                ['name_ar', 'name_en' , 'date' , 'time', 'actions'], '',
                {
        ...
        function deleteEvent(item) {
            ban(item, '{{url( (($locale === 'ar') ? $locale : '') . '/event/delete')}}', {
